#!/usr/bin/env python3
"""
magnesite_conveyor_usb_cam.py

Same logic as magnesite_conveyor_rendezvous.py but reads directly from a
USB camera (cv2.VideoCapture) instead of the RealSense ROS topics.

Depth is not available, so 3D coordinates are estimated by assuming the belt
surface is at a fixed, known distance from the camera (BELT_DEPTH_M).
Camera intrinsics and pose relative to robot base_link are configured below.
...
# ============================================================
CAMERA_DEVICE = 2          # See3CAM_CU27 USB camera (/dev/video2)
CAMERA_WIDTH  = 1280
CAMERA_HEIGHT = 720

# ---- Camera image settings (set None to leave at driver default) ----
CAM_BRIGHTNESS  = None     # typically  0–255
CAM_CONTRAST    = None     # typically  0–255
CAM_SATURATION  = None     # typically  0–255
CAM_HUE         = None     # typically -180–180
CAM_SHARPNESS   = None     # typically  0–255
CAM_GAIN        = None     # typically  0–255
CAM_EXPOSURE    = None     # manual exposure value (e.g. -7 to -1 for UVC)
CAM_AUTO_EXPOSURE = None   # 1 = manual, 3 = auto (UVC standard)
CAM_WB_TEMP     = None     # white-balance colour temperature in Kelvin
CAM_AUTO_WB     = None     # 0 = manual, 1 = auto

# Camera intrinsics — measure with cv2 calibration or use spec-sheet values.
# For a generic 1080p USB camera at 1280x720 a rough starting point is shown.
CAM_FX = 900.0             # focal length X in pixels
CAM_FY = 900.0             # focal length Y in pixels
CAM_CX = CAMERA_WIDTH  / 2.0
CAM_CY = CAMERA_HEIGHT / 2.0

# Distance from the camera optical centre to the belt plane (in metres).
# Measure with a ruler; this replaces the RealSense depth image.
BELT_DEPTH_M = 0.60

# Camera pose in robot base_link frame.
# CAM_TRANSLATION = [x, y, z] in metres from base_link origin to camera.
# CAM_ROTATION_RPY = [roll, pitch, yaw] in DEGREES (camera→base_link rotation).
# Adjust to match your physical setup.
CAM_TRANSLATION  = [0.0,  0.255, 0.60]   # camera is 255mm in Y, 600mm up
CAM_ROTATION_RPY = [180.0, 0.0,  0.0]   # camera pointing downward (flip Y,Z)
...
# Robot / push geometry
# ============================================================
Y_PARK = -0.099            # EE Y at push-start (base_link)
...
def _build_cam_to_base(translation, rpy_deg):
    """Build a 4x4 homogeneous transform from camera frame to base_link."""
...
def pixel_to_base_link(px, py, depth_m, fx, fy, cx, cy, T_cam_base):
    """Project a pixel at known depth into base_link frame."""
...
# Node
# ============================================================
class MagnesiteConveyorUsbCam(Node):
    def __init__(self):
        super().__init__('magnesite_conveyor_usb_cam')
        ...
        # Pre-build camera → base_link transform
        ...
        if not self.cap.isOpened():
            ...
            raise RuntimeError(f"Camera device {CAMERA_DEVICE} not found")
        self.cap.set(cv2.CAP_PROP_FRAME_WIDTH,  CAMERA_WIDTH)
        self.cap.set(cv2.CAP_PROP_FRAME_HEIGHT, CAMERA_HEIGHT)

        _cam_settings = [
            (cv2.CAP_PROP_AUTO_EXPOSURE,   CAM_AUTO_EXPOSURE,  "auto_exposure"),
            (cv2.CAP_PROP_EXPOSURE,        CAM_EXPOSURE,       "exposure"),
            (cv2.CAP_PROP_BRIGHTNESS,      CAM_BRIGHTNESS,     "brightness"),
            (cv2.CAP_PROP_CONTRAST,        CAM_CONTRAST,       "contrast"),
            (cv2.CAP_PROP_SATURATION,      CAM_SATURATION,     "saturation"),
            (cv2.CAP_PROP_HUE,             CAM_HUE,            "hue"),
            (cv2.CAP_PROP_SHARPNESS,       CAM_SHARPNESS,      "sharpness"),
            (cv2.CAP_PROP_GAIN,            CAM_GAIN,           "gain"),
            (cv2.CAP_PROP_AUTO_WB,         CAM_AUTO_WB,        "auto_wb"),
            (cv2.CAP_PROP_WB_TEMPERATURE,  CAM_WB_TEMP,        "wb_temp"),
        ]
        for prop, value, name in _cam_settings:
            if value is not None:
                ok = self.cap.set(prop, value)
                actual = self.cap.get(prop)
                self.get_logger().info(
                    f"Camera {name}: set={value} actual={actual} {'OK' if ok else 'FAILED'}")

        self.get_logger().info(
            f"USB camera opened: device={CAMERA_DEVICE} "
        ...
        # Camera live-adjust state
        # (name, cv2_prop, step, min, max)
        self._cam_props = [
            ('brightness',  cv2.CAP_PROP_BRIGHTNESS,      5,    0,  255),
            ('contrast',    cv2.CAP_PROP_CONTRAST,         5,    0,  255),
            ('saturation',  cv2.CAP_PROP_SATURATION,       5,    0,  255),
            ('hue',         cv2.CAP_PROP_HUE,              5, -180,  180),
            ('sharpness',   cv2.CAP_PROP_SHARPNESS,        5,    0,  255),
            ('gain',        cv2.CAP_PROP_GAIN,             5,    0,  255),
            ('exposure',    cv2.CAP_PROP_EXPOSURE,          1,  -13,    0),
            ('wb_temp',     cv2.CAP_PROP_WB_TEMPERATURE, 100, 2800, 6500),
        ]
        self._cam_prop_idx = 0
        ...
        cv2.namedWindow('Magnesite USB Cam', cv2.WINDOW_NORMAL)
        cv2.resizeWindow('Magnesite USB Cam', self.DISPLAY_W, self.DISPLAY_H)
        cv2.setMouseCallback('Magnesite USB Cam', self.mouse_callback)
    ...
    # ----------------------------------------------------------------
    # Camera-space 3D projection (replaces RealSense depth + TF2)
    # ----------------------------------------------------------------
    def _pixel_to_base_link(self, px: float, py: float) -> Tuple[float, float, float]:
        """Map a pixel in the display frame back to the camera frame then base_link."""
        # px/py are in the display (DISPLAY_W x DISPLAY_H) coordinate space,
        # which is the zoomed view; remap to original camera pixel first
        # (caller passes already-remapped native pixel coordinates)
        return pixel_to_base_link(px, py, BELT_DEPTH_M,
                                  CAM_FX, CAM_FY, CAM_CX, CAM_CY,
                                  self.T_cam_base)

    def _get_edge_x_base_link(self, idx, dets, zf, crop_rect) -> Optional[Tuple]:
        """Estimate (edge_x, bl_y, bl_z) from bounding box / mask in pixel space."""
        def z2n(zx, zy):
            # display pixel → original camera pixel
            ox, oy = zoomed_pixel_to_original(zx, zy, self.zoom, crop_rect,
                                              self.DISPLAY_W, self.DISPLAY_H)
            # scale from display resolution to actual camera resolution
            cx_ = ox * CAMERA_WIDTH  / self.DISPLAY_W
            cy_ = oy * CAMERA_HEIGHT / self.DISPLAY_H
            return cx_, cy_
        ...
        if has_mask:
            ...
            if len(mxs) == 0:
                ...
                cx,   cy   = z2n(float(np.mean(mxs)), float(np.mean(mys)))
                bll = self._pixel_to_base_link(nl_x, nl_y)
                blr = self._pixel_to_base_link(nr_x, nr_y)
                blc = self._pixel_to_base_link(cx,   cy)
    ...
    # ----------------------------------------------------------------
    # Main loop (timer-driven)
    # ----------------------------------------------------------------
    def loop(self):
        ret, full_frame = self.cap.read()
        if not ret:
            self.get_logger().warn("Failed to read frame from USB camera", throttle_duration_sec=2.0)
            return
        ...
        ann[5:140, 5:740] = (ann[5:140, 5:740] * 0.4).astype(np.uint8)
        self.fps_monitor.stop()
        cv2.putText(ann,
            f"FPS:{self.fps_monitor.getFPS():.0f} | "
            f"Infer:{self.inference_ms:.0f}ms | "
            f"Res:{self.inference_size} | ZOOM:{self.zoom:.1f}x | USB-CAM",
        ...
        _cp_name, _cp_prop, _cp_step, _cp_lo, _cp_hi = self._cam_props[self._cam_prop_idx]
        _cp_val = self.cap.get(_cp_prop)
        cv2.putText(ann,
            f"CAM [{self._cam_prop_idx+1}/{len(self._cam_props)}] "
            f"{_cp_name}: {_cp_val:.0f}  (TAB=cycle  [=dec  ]=inc  range {_cp_lo}..{_cp_hi})",
            (15, 125), cv2.FONT_HERSHEY_SIMPLEX, 0.44, (255, 180, 50), 1)

        cv2.imshow('Magnesite USB Cam', ann)
        ...
        if key in (ord('q'), 27):
        ...
        elif key == 9:  # TAB — cycle camera property
            self._cam_prop_idx = (self._cam_prop_idx + 1) % len(self._cam_props)
            name, prop, *_ = self._cam_props[self._cam_prop_idx]
            val = self.cap.get(prop)
            self.get_logger().info(f"[CAM] selected: {name} = {val:.0f}")

        elif key == ord(']'):  # increase selected camera property
            name, prop, step, lo, hi = self._cam_props[self._cam_prop_idx]
            cur = self.cap.get(prop)
            nv  = min(cur + step, hi)
            self.cap.set(prop, nv)
            actual = self.cap.get(prop)
            self.get_logger().info(f"[CAM] {name}: {cur:.0f} → {actual:.0f}")

        elif key == ord('['):  # decrease selected camera property
            name, prop, step, lo, hi = self._cam_props[self._cam_prop_idx]
            cur = self.cap.get(prop)
            nv  = max(cur - step, lo)
            self.cap.set(prop, nv)
            ...
            print(
                ...
                "  f     force-fire\n"
                "  TAB   cycle camera property\n"
                "  [/]   decrease/increase selected camera property\n"
                "  q     quit\n")
